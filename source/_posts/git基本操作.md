...
[官方教程](https://git-scm.com/download/linux)

### git配置github与gitlab同时使用

1. `git bash` 运行命令`ls -al ~/.ssh`，查看.ssh文件下是否有生成秘钥文件
2. 如果目录下没有`.ssh`文件，使用`mkdir ~/.ssh`生成`.ssh`文件，同时`chmod 700 ~/.ssh`给权限
3. 如果有`.ssh`文件，运行`cd ~/.ssh`到`.ssh`文件下，输入`ls`命令查看秘钥文件
4. 生成秘钥文件
...
```js
  ...
  #gitlab
  Host gitlab
        HostName xiaoyi.com //写自己公司的gitlab网址
...
git config --global user.name "xiaoyi"          # 修改全局名字
git config --global user.email "963132341@qq.com"  # 修改全局邮箱
...
```bash
ssh-keygen -t rsa -C '963132341@qq.com' # 生成密钥  
ssh-keygen -t rsa -C "963132341@qq.com" -f ~/.ssh/ww_rsa # 指定生成目录文件名字
